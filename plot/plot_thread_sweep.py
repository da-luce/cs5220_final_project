"""
Throughput vs OpenMP thread count, one curve per variant.
...
def main():
    ...
    print(f"{'variant':>10} {'THREADS':>8} {'BATCH':>6} {'med_batch_ms':>14} {'throughput_g/s':>16}")
    for v in variants:
    ...
    for v in variants:
        ...
        ax.plot(ts, thr, "o-", color=variant_color[v], label=v)
        peak = peaks.get(v)
        if peak is not None:
